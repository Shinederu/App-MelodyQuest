...
test("le service worker limite son cache aux fichiers du site", async () => {
  ...
  assert.equal(assetIndex.version, "20260915-notoriety-slider");
...
test("la page déclare le manifeste et enregistre la PWA", async () => {
  ...
  assert.match(index, /20260915-notoriety-slider/);
  assert.match(appController, /registerPwa\(\)/);
  assert.match(appController, /ASSET_VERSION = "20260915-notoriety-slider"/);
  assert.match(serviceWorker, /RELEASE = "20260915-notoriety-slider"/);
  assert.match(assetGenerator, /RELEASE = "20260915-notoriety-slider"/);
});
