...
- cache-bust JS courant: `20260619-autoplay-return-lobby`;
- cache-bust CSS courant: `20260619-autoplay-return-lobby`;
...
Passe TV passif 2026-06-17: le cache-bust `20260617-passive-tv-cleanup` masque le classement/joueurs sur la vue TV quand le salon est en mode `autoplay`. La partie passive garde une interface de lecture pure: video, progression de manche, phase et solution, sans score ni liste de participants.

Passe categorie visible 2026-06-17: le cache-bust `20260617-category-default-visible` active par defaut l'option `Categorie visible` sur les nouveaux salons actifs et passifs. Le createur peut toujours la desactiver dans les reglages du lobby.

Passe barre de jeu 2026-06-17: le cache-bust `20260617-game-toolbar-cleanup` regroupe `Proposer une correction`, le volume et le pourcentage sous le lecteur. Le lecteur et l'API restent inchanges.

Passe plein ecran lancement 2026-06-17: le cache-bust `20260617-launch-fullscreen` demande le plein ecran au clic de lancement du salon, pour les modes actif et passif. Le plein ecran reste best-effort car certains navigateurs le refusent ou ne le supportent pas.

Passe admin catalogue 2026-06-17: le cache-bust `20260617-admin-workflow` refond la page `#/management-suggestions` avec edition/application directe des propositions, et ajoute `#/management-answers` pour analyser les reponses saisies par les joueurs.

Passe retour lobby passif 2026-06-19: le cache-bust `20260619-autoplay-return-lobby` renvoie automatiquement les salons en mode passif vers `#/lobby` a la fin de toutes les manches.
...
- Suggestions joueurs: alias, correction URL/libelle/artiste/licence, proposition publique de nouvelle musique, edition admin et application directe au catalogue.
- Analyse admin des reponses joueurs: essais rates/groupes frequents et historique recent pour reperer des alias, corrections ou idees de musiques.
